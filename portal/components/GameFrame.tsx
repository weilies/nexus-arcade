...
export function GameFrame({ slug, gameName, matchId }: GameFrameProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col">
      <div className="font-pixel text-sm tracking-wide py-1.5 px-3 border-b flex items-center justify-between gap-2"
           style={{
        ...
        <span className="font-semibold truncate" style={{ color: 'var(--neon-cyan)' }}>▶ {gameName.toUpperCase()}</span>
        <button
          onClick={() => iframeRef.current?.requestFullscreen()}
          className="flex items-center gap-1.5 px-3 py-1.5 hover:opacity-80 transition-opacity border rounded"
          style={{
            color: 'var(--neon-cyan)',
